...
class NFA:
    ...
    def __init__(self, letter):
        ...
        # -G     : list[src] = dict{letter:{set of dst nodes}}]
        # _G[0]  : start node
        # _G[-1] : terminal node
        if letter == "1":
            self._regex = ""
    ...
    def iterate(self):
        ...
        self.addEdge("1", 0, self.finish())
        self._G.append({})
        self.addEdge("1", self.finish() - 1, self.finish())
    ...
    def condensate(self):
        ...
        for nodeDict in self._G:
            graph.append(nodeDict["1"] if "1" in nodeDict else [])
        ...
        for i in range(len(self._G)):
            if "1" in self._G[i]:
                self._G[i]["1"].discard(i)
                if len(self._G[i]["1"]) == 0:
                    del self._G[i]["1"]
    ...
    def removeEpsilonEdges(self):
        ...
        for i in range(len(self._G) - 1):
            while "1" in self._G[i]:
                nodes = list(self._G[i]["1"])
                self._G[i]["1"] = set()
                for j in nodes:
                    if j == self.finish():
                        self._G[i]["$"] = {j}
    ...
    def numTerminals(self):
        ...
        return sum(1 for nodeDict in self._G if ("1" in nodeDict and self.finish() in nodeDict["1"]) or ("$" in nodeDict and self.finish() in nodeDict["$"]))
